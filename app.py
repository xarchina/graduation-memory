...
# ===================== Supabase 全局单例（纯REST接口，无数据库密码、无报错参数） =====================
@st.cache_resource
...
if st.session_state.login_username is None:
    ...
    # ===================== 1. 班级留言墙（hover显示点赞回复、回复折叠、❤点赞按钮、无多余边框） =====================
    if nav_menu == "班级留言墙":
